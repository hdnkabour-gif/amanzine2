...
import { attemptKey, endAttempt } from '../lib/attemptKey';    // نقرتان ⇒ طلبٌ واحد
...
function ServiceModal({p,onClose,currency,storeInfo,userId}:{p:SProduct;onClose:()=>void;currency:string;storeInfo:StoreInfo;userId:string}) {
  ...
  const submit=async()=>{
    ...
    setLoading(true);
    try{
      const r=await fetch('/api/orders/public',{method:'POST',headers:{'Content-Type':'application/json','Idempotency-Key':attemptKey('service-order')},body:JSON.stringify({
      ...
      endAttempt('service-order');   // نجحت المحاولة ⇒ التالي محاولةٌ أخرى
...
export default function Storefront() {
  ...
  return (
    <div dir={rtl?'rtl':'ltr'} style={{minHeight:'100dvh',background:DS.bg,color:DS.textPrimary,fontFamily:'Tajawal,system-ui,sans-serif',/* ثلاثةُ عناصرَ ثابتةٍ تتراكم أسفلَ الشاشة (السلّة · المساعد · العجلة) وارتفاعُها مجتمعةً ~١٤٠px. الحشوُ كان ٨٠ فتختفي آخرُ صفٍّ من المنتجات خلفها — وهو ما ظهر في لقطة الشاشة. */paddingBottom:'calc(150px + env(safe-area-inset-bottom, 0px))',position:'relative'}}>
