...
export function readFacts(text: string): Fact[] {
  ...
  // **«نتا» تصلح لمن يكون، لا لما يملك.**
  //
  //   من شاشة صاحب المشروع: «bghit nebi3 bikala» ⇒ **«نتا درّاجة هوائيّة
  //   وباغي تعلن»**. والصيغةُ كُتبت لحرفةٍ («نتا سبّاك وباغي تعلن») ثمّ
  //   وردتها سلعةٌ فقالت للإنسان إنّه درّاجة.
  //
  //   والفرقُ مقروءٌ سلفًا في السطر أعلاه: ما جاء من `profession` **هو
  //   المتكلّمُ نفسُه**، وما جاء من غيره **شيءٌ يملكه**. فلا حاجةَ إلى قائمةٍ
  //   جديدة — الفرقُ في مصدر الكلمة لا في الكلمة.
  const isWho = !!u.profession?.label && u.profession.label === raw;
  ...
  if (what) {
    out.push({
      ...
      say: st === 'offer' ? (isWho ? `نتا ${what} وباغي تعلن` : `باغي تعرض ${what}`)
