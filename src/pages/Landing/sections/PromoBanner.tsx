...
export default function PromoBanner() {
  ...
  return (
    <div style={{ position: 'relative', background: `linear-gradient(135deg, ${C.orange}, ${C.purple})`, color: '#fff', padding: '9px 40px', textAlign: 'center', fontSize: 13, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 14, flexWrap: 'wrap' }}>
      <span>{tx('promo')}</span>
      <Link to={isAuthed ? '/dashboard' : '/login'} style={{ background: 'rgba(255,255,255,0.22)', border: '1px solid rgba(255,255,255,0.4)', color: '#fff', borderRadius: 99, padding: '3px 14px', fontSize: 12, fontWeight: 800, textDecoration: 'none', whiteSpace: 'nowrap' }}>{tx('promoCta')}</Link>
