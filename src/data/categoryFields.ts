...
export const CATEGORY_FIELDS: Record<string, CatFieldDef[]> = {
  ...
  // ── 👕 ملابس رجالية ──────────────────────────────────────────
  // ── ملابسٌ عامّة ─────────────────────────────────────────────
  // فئةُ المتجر العامّ: من يبيع «حوايج» بلا تحديدِ رجالٍ أو نساءٍ أو أطفال.
  // حقولُها هي المشترَكُ بين الثلاث — لا نسخةٌ رابعةٌ من كلٍّ منها.
  clothes: [
    { id: 'audience', label: 'لمن؟', type: 'multiselect', required: true,
      options: ['رجال', 'نساء', 'أطفال', 'رُضّع', 'للجميع'] },
    { id: 'subtype', label: 'نوع اللباس', type: 'select', required: true,
      options: ['قميص', 'تيشيرت', 'سروال', 'جينز', 'فستان', 'جلابة', 'قفطان', 'تكشيطة',
        'جاكيت', 'معطف', 'هودي', 'بيجامة', 'حجاب', 'طقم', 'أخرى'] },
    { id: 'fabric', label: 'الخامة', type: 'select',
      options: ['قطن', 'صوف', 'كتان', 'جينز', 'جلد', 'بوليستر', 'حرير', 'مخمل', 'شيفون', 'مخلوط'] },
    { id: 'occasion', label: 'المناسبة', type: 'multiselect',
      options: ['يومي', 'عمل', 'رسمي', 'كاجوال', 'عرس', 'عيد', 'رمضان', 'رياضي', 'بيت'] },
    { id: 'season', label: 'الموسم', type: 'multiselect', options: SEASONS },
    { id: 'itemCondition', label: 'حالة القطعة', type: 'select', required: true,
      options: ['جديد بالعلامة', 'جديد بلا علامة', 'شبه جديد', 'مستعمل نظيف', 'مستعمل'] },
    { id: 'brandTier', label: 'الماركة', type: 'select',
      options: ['ماركة أصلية', 'تقليد ممتاز', 'بلا ماركة', 'صناعة محلية'],
      hint: 'الصدقُ هنا يمنع نزاعًا بعد البيع' },
    { id: 'careLabel', label: 'إرشادات العناية', type: 'select',
      options: ['غسيل عادي', 'غسيل يدوي', 'تنظيف جاف فقط', 'بلا تبييض'] },
  ],
  ...
  // ── 🛠️ خدمات ─────────────────────────────────────────────────
  service: [
    // ── حُذفت ثلاثةُ حقول، وكلٌّ منها كان يخالف قانونًا قائمًا ──────
    //
    //   `serviceType` — قائمةٌ من ٢٣ خيارًا تنافس معرفةً من ١٤٧ مفهومًا.
    //     قِستُها: **٢٢ مهنةً فقط من ١٤٧ تجد اسمَها فيها**؛ فالميكانيكيُّ
    //     والجزّارُ وصاحبُ المطعم والمخبزةِ يُجبَرون على «أخرى». وهي فوق ذلك
    //     **نفسُ `profession`** الذي تملؤه المعرفةُ تلقائيًّا: حقلان لحقيقةٍ
    //     واحدة. (قاعدةٌ لا قائمة · حقلٌ واحدٌ اسمٌ واحد)
    //
    //   `serviceLocation` — **إجباريٌّ بلا مستهلكٍ واحد** في كلّ التطبيق،
    //     وهو نفسُه `mobile` («كتمشي عند الزبون؟») في قالب الخدمة.
    //     (كلُّ سؤالٍ له مستهلك)
    //
    //   `minBooking` و`teamSize` — صفرُ مستهلكين كذلك.
    //
    //   والثمنُ يُدفَع في أوّل خمس دقائق: التاجرُ يجيب عن أسئلةٍ إجباريّةٍ
    //   تختفي أجوبتُها، فيظنّ أنّ التطبيقَ استمارةٌ أخرى.
