...
function hitsArabic(term: string, text: string): boolean {
  ...
  // **والتاءُ المربوطةُ تُكتَب بثلاثة أحرف** — نفسُ قبولِ `matchWord`.
  //
  //   رآه صاحبُ المشروع: «بغيت **كسوى** لبنتي» ⇒ «كتقلّب على شي حدّ» ثمّ
  //   «الماء / السباكة». والمطبِّعُ يحوّل `ة⇒ه` و`ى⇒ي`، فـ«كسوة»⇒«كسوه»
  //   و«كسوى»⇒«كسوي» — كلمتان في نظر المحرّك.
  //
  //   ولا يُطوى في المطبِّع لأنّه يُخمِّن: «عندى» ياءٌ لا تاءٌ مربوطة، وطيُّها
  //   قلب «عندي محل ديال الخضرة» إلى `unknown` — أسقطه حارسٌ قائمٌ فورًا.
  //   فالمصطلحُ وحدَه هو من يعلن قبولَه، وكلامُ الإنسان لا يُمَسّ.
  //
  //   وهذه **خامسُ نسخةٍ** لحدّ الكلمة في هذا المستودع. وُحِّدت `hitsWord`
  //   و`matchWord` في `normalize`، وبقيت هذه لشرطٍ خاصٍّ بها (المصطلحُ
  //   القصيرُ يُشترَط له طرفاه). فالقبولُ يُنسَخ هنا عمدًا — ويحرسه اختبارٌ
  //   يقارن السلوكَين، فلا يفترقان بصمت.
  const start = `(^|\\s)${PREFIX}${/ه$/.test(term) ? esc(term.slice(0, -1)) + '[هي]' : esc(term)}`;
