...
const SEEK_MARKS = [
  ...
  // صيغةُ الأمر بالإعطاء: «عطيني منتوج بأقلّ من ٣٠ درهم» — طلبٌ صريحٌ لا
  // يخفى على إنسان، وكان يُقرأ بلا اتّجاه فيُقابَل بـ«ما فهمتش». والمغربيُّ
  // يأمر بالإعطاء أكثرَ ممّا يقول «بغيت» حين يُخاطب بائعًا.
  'عطيني', 'عطينا', 'عطيوني', 'جيب ليا', 'جيبو ليا', 'ورّيني شي', 'بيّن ليا شي',
  // ── **والقُربُ طلبٌ لا عرض** ────────────────────────────────────
  //
  //   قِيس على اقتراحات التطبيق نفسِه: «مطعم قريب» ⇒ `create_store`،
  //   و«أحتاج حرفي» ⇒ `create_service`. أي أنّ مَن يضغط زرَّ «🍽️ مطعم»
  //   يُساق إلى **إنشاء مطعم** — والزرُّ من عندنا لا من عنده.
  //
  //   والسببُ أنّ «قريب» ليست علامةَ طلبٍ، فيبقى الاتّجاهُ `unknown` ثمّ
  //   يُخمَّن عرضًا. **ولا أحدَ يقول «مطعم قريب» وهو يفتح مطعمًا** — الكلمةُ
  //   تصف مسافةً بينه وبين شيءٍ يريد بلوغَه، وهذا طلبٌ بتعريفه.
  'قريب', 'قريبة', 'قريبه', 'حدايا', 'حداي', 'بحدايا', 'فحينا', 'فالحي ديالي',
  'قربي', 'بقربي', 'اللي حدايا', 'اللي قريب',
];
...
const COMPLAINT = [
  ...
  // **الصيغةُ المجرّدة كما الانعكاسيّة.** «تخربقات» كانت تُقرأ شكوى و«خربات»
  // لا — وهما كلمةٌ واحدةٌ بصيغتَين، والمغربيُّ يقول الثانيةَ أكثر:
  // «الماكينة ديال الخياطة **خربات**». فكانت شكوى تُقرأ بلا اتّجاه، فلا
  // يعرف المحرّكُ أنّ صاحبَها **يطلب** إصلاحًا.
  'خربات', 'خرباو', 'خربو', 'تهرست', 'تهرسات', 'طاحت', 'وقفات',
];
...
// أفعالُ العرض بصيغة المتكلّم — تغلب أداةَ الطلب حين تجتمعان.
  // **و«بنيع» خطأٌ مطبعيٌّ مقيس**: قلبُ أوّل حرفَين في «نبيع» — كتبها صاحبُ
  //   المشروع في «بغيت **بنيع** كسوة»، فقُرئت **شراءً** وهو يبيع. وتُضاف
  //   سطحًا صريحًا لا بمطابقةٍ ضبابيّة: مصحِّحٌ يخمّن يُصلح خطأً ويصنع عشرة.
const SELF_OFFER = ['نبيع', 'كنبيع', 'نسوق', 'نسوّق', 'باغي نبيع', 'بغيت نبيع', 'بنيع', 'بغيت بنيع'];
...
/** فعلُ معاملةٍ يلحقه ضميرُ المفعول: «نكريها» · «نبيعه» · «نسوّقهم». */
const OWNED_DEAL = /(^|\s)(ن|كن)(كري|بيع|سوق)(ها|هما|هم|ه)(\s|$)/;
...
/**
 * **والقصيرُ يحتاج حدًّا — وإلّا صارت كلُّ شقّةٍ عطبًا.**
 *
 *   قِيس حرفيًّا: `DEFECT` فيه «**شق**» (صدع)، و«**شقة**» تحتويها. فكانت
 *   كلُّ جملةٍ فيها شقّةٌ تُقرأ **شكوى**، والشكوى طلب:
 *
 *       «عندي شقة **للبيع**»        ⇒ طلب   (وهو يعرض!)
 *       «بغيت **نبيع** شقة»         ⇒ طلب   (وهو يبيع!)
 *       «عندي شقة باغي نكريها»      ⇒ طلب   (وهو يُكري!)
 *
 *   فصاحبُ الشقّة يُقرأ باحثًا عنها. و«دار» تنجو لأنّ لا صدعَ في حروفها —
 *   العطبُ يخصّ الشقق وحدَها، ولذلك عاش طويلًا.
 *
 *   وهذا **خامسُ** مواضع مصرف الكلمة العامّة بعد فهرس المفاهيم ورسم الأعراض
 *   ومطابِق المدن وجدول البضائع. والعلاجُ هو هو، وقد جُرِّب: تشديدُ **كلّ**
 *   المطابقة إلى حدّ الكلمة أسقط **٢٥٢** اختبارًا — لأنّ العربيّة تلحق
 *   («خربان» يجب أن تُطابق «خربانة»). فالحدُّ للقصير وحدَه، وهي نفسُ قاعدة
 *   `knowledge.SHORT` المكتوبةِ منذ «تصبينة ⇒ بقّال».
 */
const SHORT_CUE = 3;

/** `t` مطبَّعٌ سلفًا؛ يُطبَّع المصطلحُ هنا مرّةً ويُحفَظ. */
const inc = (t: string, w: string) => {
  const n = NZ(w);
  return n.length <= SHORT_CUE ? hitsWord(n, t) : t.includes(n);
};
...
function detectStance(t: string, c?: { stance?: { offer?: string[]; seek?: string[] } }): Stance {
  ...
  // **والضميرُ يعود على المملوك، فهو عرضٌ لا طلب.**
  //
  //   «عندي شقة باغي **نكريها**» — الهاءُ تعود على شقّةٍ يملكها، فهو يُكري
  //   لا يكتري. وكانت تُقرأ طلبًا لأنّ «باغي» أداةُ طلب، ولأنّ عائلةَ البيع
  //   مُعلَنةٌ كاملةً في `SELF_OFFER` («نبيع» تُطابق «نبيعها») ولا مقابلَ لها
  //   في الكراء — فـ«باغي نبيعها» تعمل و«باغي نكريها» لا. حرفٌ واحدٌ يفرّق
  //   بين تاجرَين.
  //
  //   والشرطُ **ملكيّةٌ + ضمير** معًا: «بغيت نكري دار» بلا «عندي» تبقى طلبًا،
  //   وهي الصواب — الدارجةُ تستعمل «نكري» للطرفَين، والمكتري أشيعُ.
  if (HAS.some(h => inc(t, h)) && OWNED_DEAL.test(t)) return 'offer';
  ...
  // **وفعلُ المتكلّم بلا أداةِ طلبٍ إخبارٌ عن عمل.** «نغسل الكوسانات» يقولها
  //   صاحبُ المغسلة يصف ما يفعل. وموضعُها **بعد** أدوات الطلب لا قبلها:
  //   «بغيت نغسل الطوموبيل» نفسُ الفعل وأداةُ طلبٍ تسبقه — فهي زبونٌ لا محلّ.
  //   الفرقُ كلمةٌ واحدة، والترتيبُ وحدَه يحفظه.
  //
  //   وكانت «نغسل الكوسانات» تُقرأ عرضًا **بمطابقةٍ عرضيّة** لا بفهم: قرينةٌ
  //   قصيرةٌ وقعت داخل حروفها. فلمّا شُدّ حدُّ القصير ظهرت عاريةً — والرقمُ
  //   القديمُ كان يعدّ صوابًا سببُه مصادفة.
  //   ويُشترَط **مفعولٌ يتبعه**: «نجار» و«نجّاد» حِرفتان تبدآن بالنون، ومَن
  //   قالهما مجرّدتَين لم يقل أيَعرض أم يطلب — فيُسأل ولا يُخمَّن. أمسكه
  //   حارسٌ قائمٌ في أوّل تشغيل.
  if (/(^|\s)ن[ء-ي]{3,}\s+[ء-ي]/.test(t)) return 'offer';
