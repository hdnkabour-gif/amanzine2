...
test('**والوثيقةُ لا تنصح بما يكسر** — `.env.example` يطابق العقدَ  [SOURCE_SHAPE]', () => {
  // وثيقةٌ متقادمةٌ ليست خطأً هيّنًا هنا: هي **أوّلُ ما يقرؤه المشغِّل**.
  //   كانت تقول «الخادمُ يستعمل TLS متسامحًا تلقائيًّا للقواعد البعيدة،
  //   وهذا يُصلح خطأ self-signed» — وصفًا لسلوكٍ أُلغي في RC-P5. ثمّ:
  //   «على Railway اضبط DATABASE_URL وحدَه ولا شيءَ غيرَه» — أي **نفسُ
  //   الإعداد الذي أنتج 503 في نشرٍ حقيقيّ**. فالنصيحةُ الخاطئةُ تسوق
  //   القارئَ إلى العطب الذي كلّف جلسةً كاملةً لتشخيصه.
  //
  //   وهذا حارسُ شكلٍ لا سلوك: يمنع الوثيقةَ من التناقض مع `dbSsl.js`،
  //   ولا يُغني عن جدول ③ب السلوكيّ فوقه.
  const env = require('fs').readFileSync(
    require('path').join(__dirname, '../.env.example'), 'utf8');
  assert.doesNotMatch(env, /tolerant TLS/i,
    '`.env.example` ما زال يَعِد بـTLS متسامح — سلوكٌ لم يعد موجودًا');
  assert.doesNotMatch(env, /set ONLY DATABASE_URL/i,
    '`.env.example` ما زال ينصح بإعدادٍ يُنتج 503 على Railway الخاصّة');
  assert.match(env, /DB_TRANSPORT=railway-private/,
    'اختفى عقدُ النقل الخاصّ من الوثيقة التي يقرؤها المشغِّل');
  assert.match(env, /RAILWAY_ENVIRONMENT_ID/,
    'الوثيقةُ لا تذكر الشرطَ الوحيدَ الذي لا يكتبه المشغِّل');
});
