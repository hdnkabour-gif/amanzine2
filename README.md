...
```bash
npm test                 # كلّ المجموعات (هي نفسُها ما يشغّله CI)
npm run build            # بناء الواجهة (Vite)
npm run lint             # فحص الأنواع — بوّابةٌ في CI لا تحذير
```

المجموعاتُ الخمس، وكلُّها بوّاباتٌ في CI:

| الأمر | ماذا يحرس |
|---|---|
| `test:brain` | فهمُ الدارجة واللاتينيّة · الغموض · الذاكرة |
| `test:constitution` | قواعدُ المنتج مفحوصةً في الكود نفسِه |
| `test:knowledge` | تعارضاتُ المرادفات · الفئاتُ الفاسدة |
| `test:architecture` | 8 قواعد: لا اسمَ شركةِ توصيلٍ في منطق الأعمال · المُولَّد مطابقٌ لمصدره · لا أصلَ مفقود |
| `cd server && npm test` | التسعير · السجلّ · المدن · القواعد · إعادة المحاولة |

**التوليد** (يُشغَّل عند تعديل قاعدة المعرفة، والحارسُ يفشل إن نُسي):

```bash
npm run gen:cities       # معجم المدن للخادم من knowledgeData.ts + geo.ts
npm run gen:variants     # مرادفات المفاهيم
```

أمان: مصادقة JWT (ساعة) + رموز تحديث دوّارة مُجزّأة (SHA‑256)، استعلامات مُعاملة (لا حقن
SQL)، تشفير أسرار الإعدادات ومفاتيح شركات التوصيل (AES‑256‑GCM)، حجبُ SSRF على روابط
الـwebhook، وحدّ معدّل على النقاط الحسّاسة.

> **حالةُ الكود الفعليّة** — ما يعمل، وما يظهر ولا يعمل، وما يعمل ولا يُرى:
> [`docs/STATE_OF_THE_CODE.md`](docs/STATE_OF_THE_CODE.md)
