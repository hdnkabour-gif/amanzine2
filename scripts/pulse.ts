...
export function measure(): Pulse {
  ...
  for (const s of ALL) {
    ...
    //   ── **والمعفاةُ تُعفى من المقام، لا من الحراسة** ──────────────
    //
    //   كان `else if`: المعفاةُ لا تُعدّ في البسط إطلاقًا. وسبرٌ أسقط ذلك —
    //   من السبع المعفاة **خمسٌ اتّجاهُها مقروءٌ صحيحًا اليوم**:
    //     «كيفاش نبيع قميص» · «واش نقدر نبيع قميص» · «يمكن نبيع القميص» ·
    //     «الى كان الثمن مناسب غادي نبيع» · «قال ليا الزبون بغيت نرجع»
    //   واثنتان وحدَهما مجهولتان بحقّ (النفيُ والماضي).
    //
    //   فكُسرت قراءةُ الاتّجاه في **كلّ** جملةٍ معفاةٍ عمدًا، فلم يتحرّك
    //   النبضُ ولا سقّاطتُه ولا العقلُ كلُّه: خمسُ قراءاتٍ صحيحةٍ بلا حارس.
    //   والاستثناءُ لم يكن يُضخّم النسبةَ — لكنّه كان **يُعمي** عنها.
    //
    //   والعقدُ الصحيح: تُعدُّ في **البسط** إن قُرئ اتّجاهُها، ولا تُرفَع من
    //   **المقام** إلّا حين يُعفي الإعفاءُ شيئًا فعلًا — أي حين تكون معفاةً
    //   **و**مجهولةً معًا. فمعفاةٌ اتّجاهُها مقروءٌ نجاحٌ كامل، وطرحُها من
    //   المقام وحدَه أعطى ٧٦/٧٣ = ١٠٤٪ — ونسبةٌ فوق المئة مقياسٌ يكذب.
    //
    //   واليومَ: ٧ معفاةٌ منها **٢ مجهولتان بحقّ** (النفيُ والماضي). فالمقامُ
    //   ٧٨ لا ٧٣، والنسبةُ ٧٦/٧٨. والسقفان يشتدّان معًا: `maxStanceMoot`
    //   ينزل من ٧ إلى ٢، و`minStanceCounted` يصعد من ٧٣ إلى ٧٨.
    const stanceMootHere = !!u.negated || u.mood?.executable === false;
    const stanceRead = !!u.stance && u.stance !== 'unknown';
    if (stanceRead) stanceKnown++;
    else if (stanceMootHere) stanceMoot++;
