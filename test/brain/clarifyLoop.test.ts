...
// ============================================================
// جملٌ من تجربةِ المالك نفسِه — «عدّةُ أشياءَ لازالت لا تُفهَم».
//   كلُّ حالةٍ هنا فشلت فعلًا قبل إصلاحها. لا أمثلةَ مُختلَقة.
// ============================================================

test('«الضو مشا» انقطاعُ كهرباءٍ عاجل — لا جملةٌ مجهولة', () => {
  for (const q of ['الضو مشا', 'مشا ليا الضو', 'الضو طفا', 'الكهربا مشات']) {
    const r = parseNeed(q);
    assert.notEqual(r.intent, 'unknown', `${q} لم يُفهَم`);
    assert.equal(r.object?.profession, 'كهربائي', `${q} → ${r.object?.profession}`);
  }
  // ولم تُكسَر الصيغةُ التي كانت تعمل.
  assert.equal(parseNeed('طاح الضو').object?.profession, 'كهربائي');
});

test('«بغيت نفتح متجر» إنشاءُ متجرٍ — لا بحثٌ عن حِرفيّ', () => {
  for (const q of ['بغيت نفتح متجر', 'بغيت ندير متجر', 'بغيت نفتح حانوت']) {
    assert.equal(parseNeed(q).intent, 'create_store', `${q} → ${parseNeed(q).intent}`);
  }
  // «عندي محل» ما زالت تعمل كما كانت.
  assert.equal(parseNeed('عندي محل وباغي نبيع فيه أونلاين').intent, 'create_store');
});

test('مَن يطلب إنسانًا لا يُلقى في السوق يبحث عن سلعة', () => {
  // «كنقلب على» في معجم `BUY` وهي في الدارجة «أبحث عن» لا «أشتري».
  for (const q of ['كنقلب على مدرس دعم', 'بغيت واحد ينقل ليا الموبيليا', 'شكون كيصلح الماكينة ديال الغسيل']) {
    assert.equal(parseNeed(q).intent, 'find_pro', `${q} → ${parseNeed(q).intent}`);
  }
});

test('وشراءُ السلعة ما زال شراءً — التمييزُ لم يبتلع الحالةَ الأخرى', () => {
