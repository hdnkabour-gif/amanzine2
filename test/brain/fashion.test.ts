...
test('السياقاتُ أنواعٌ لا قائمةٌ واحدة', () => {
  // الحالةُ والأصالةُ والمنشأُ ونوعُ البيع أوصافٌ أيضًا — وكانت تُسأل دائمًا
  // ولو قالها التاجرُ في السطر نفسِه. و«صنع في المغرب» كانت أسوأ: تُقرأ
  // **بحثًا عن حرفيّ** لأنّ «صنع» فعل.
  const cases: [string, string, string][] = [
    ['تلفون مستعمل',        'condition',     'مستعمل'],
    ['تلفون جديد',          'condition',     'جديد'],
    ['صباط أصلي',           'authenticity',  'أصلي'],
    ['صباط كوبي',           'authenticity',  'تقليد'],
    ['جلابة صنع في المغرب', 'origin',        'المغرب'],
    ['حوايج مستوردة',       'origin',        'مستورد'],
    ['كنبيع بالجملة',       'transaction',   'بالجملة'],
    ['بالتقسيط',            'transaction',   'بالتقسيط'],
  ];
  for (const [need, key, value] of cases) {
    const hit = extractContexts(need).find(c => c.key === key);
    assert.ok(hit, `«${need}» لم يُلتقَط منه ${key}`);
    assert.equal(hit!.value, value);
    assert.ok(hit!.because, `${key} بلا سبب`);
  }
});

test('«للبيع» و«للكراء» نيّةٌ لا سياق — لا مصدرَ ثانيًا لهما', () => {
  // اقتُرح جعلُهما سياقَ معاملة. لكنّهما يُغيّران **القالبَ نفسَه**: الكراءُ
  // له ثمنٌ يوميٌّ وضمانةٌ وتأمين. وضعُهما في السياقات أيضًا يُنشئ حقيقتين.
  const terms = allContextTerms().map(t => t.toLowerCase().trim());
  for (const t of ['للبيع', 'للكراء', 'للايجار', 'للإيجار'])
    assert.ok(!terms.includes(t),
      `«${t}» صارت سياقًا وهي نيّة — مصدران لشيءٍ واحد`);
});
