...
test('وجملُ القبول تبلغ الذكاءَ — وهي التي بُنيت البوّابةُ لها', () => {
  for (const s of ['بغيت نمشي لطنجة', 'بغيت نمشي الحي عندي غير 10 دراهم',
                   'بغيت شي حاجة', 'بغيت شي واحد']) {
    assert.equal(gate(s), true, `«${s}» لا تبلغ الذكاءَ ولا تفهمها القواعد`);
  }
});

test('**وما صارت القواعدُ تفهمه لا يُصعَّد** — والتصعيدُ نقصٌ يُسدّ لا غاية', () => {
  // «مكرهتش شي سندويش طون بالحرور» كانت ثالثةَ جملِ القبول، وكانت تُصعَّد.
  //   ثمّ تبيّن أنّ عطبَها **إملائيٌّ لا فهميّ**: «سندويتش» معروفةٌ و«سندويش»
  //   لا. فلمّا سُدّ النقصُ في المعرفة سقط التصعيد — وهذا هو الصواب:
  //   الذكاءُ الخارجيُّ يسدّ ما نجهله، ولا يُشترى به ما نستطيع أن نعرفه.
  //   والاختبارُ يُثبّت الاتّجاه: كلَّما تعلّمت القواعدُ قلّ النداءُ البعيد.
  const s = 'مكرهتش شي سندويش طون بالحرور';
  assert.equal((understand(s) as any).service, 'restaurant', 'رجعت الجملةُ مجهولة');
  assert.equal(gate(s), false, 'تُرسَل للذكاء وقد صارت مفهومة');
});
...
test('والفراغُ يُملأ — وهذا هو الغرض', () => {
  // جملةٌ **ما زالت** مجهولةً للقواعد — وهي شرطُ الاختبار لا زينتُه.
  const base = understand('بغيت شي حاجة') as any;
  assert.equal(base.service, undefined, 'صارت الجملةُ مفهومةً — اختر أخرى مجهولة');
