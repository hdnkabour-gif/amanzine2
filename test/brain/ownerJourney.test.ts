...
// ── ④ السؤالُ المغلقُ لا يستبعد جوابَ صاحبِه ──────────────────
test('**من حدّد فقد أجاب — ولو بما لا نعرفه**', () => {
  // «عندي الماكينة ديال **الصابون** خاسرة» ⇒ «ماكينة ديال شنو؟ الخياطة،
  // الحلاقة، ولا القهوة؟». قال «ديال الصابون» في الجملة نفسِها، فسُئل
  // سؤالًا يعرض ثلاثةً **تستبعد جوابَه** — ولا مخرجَ له منه.
  const u = understand('عندي الماكينة ديال الصابون خاسرة') as any;
  assert.equal(u.ambiguity, undefined, 'سُئل سؤالٌ مغلقٌ لا يحوي ما قاله');
});

test('والغموضُ يبقى حيث لا تخصيص — الحدُّ ليس صممًا', () => {
  // نصفُ الحارس: إلغاءُ السؤال حيث لا تخصيصَ يُعيد التخمينَ الذي وُلد لمنعه.
  assert.equal((understand('عندي ماكينة خاسرة') as any).ambiguity?.term, 'ماكينه');
  assert.equal((understand('بغيت فران') as any).ambiguity?.term, 'فران');
  // والقرينةُ المعروفةُ تحسم كما كانت — لا تُسأل ولا تُهمَل.
  assert.equal((understand('عندي ماكينة ديال الخياطة خاسرة') as any).service, 'sewing_machine_repair');
});

// ── ⑤ خطأٌ مطبعيٌّ قلب البائعَ مشتريًا ────────────────────────
test('«بنيع» قلبُ حرفَين في «نبيع» — والبائعُ كان يُقرأ مشتريًا', () => {
  const u = understand('بغيت بنيع كسوة') as any;
  assert.equal(u.stance, 'offer', '«بغيت بنيع كسوة» تُقرأ شراءً وهو يبيع');
  // ويُضاف سطحًا صريحًا لا بمطابقةٍ ضبابيّة: مصحِّحٌ يخمّن يُصلح خطأً ويصنع
  // عشرة. والشراءُ الصحيحُ لم ينكسر.
  assert.equal((understand('بغيت نشري كسوة') as any).stance, 'seek');
});
