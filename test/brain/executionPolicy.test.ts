...
test('والباحثُ لا تُنتزَع منه وجهتُه — الحدُّ معلَن', () => {
  // `FIND_PROVIDER.page = 'home'` يقول **أين تعيش القدرة**، لا «أين يُساق
  // الآن». ووجهةُ الباحث `/market?q=…` أنفعُ منها. فلو مُلئت الوجهةُ لكلّ
  // قدرةٍ لأخرجنا الباحثَ من نتائجه إلى صفحةٍ فارغة.
  for (const s of ['بغيت ميكانيكي فكازا', 'بغيت نبيع سباط', 'عندي محل ديال الخضرة']) {
    assert.equal(decideFor(s).d.dest, undefined, `«${s}» انتُزعت وجهتُها`);
  }
});
