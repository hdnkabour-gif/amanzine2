...
test('الفعلُ الإداريُّ يُساق إلى باب قدرته لا إلى السوق', () => {
  const CASES: [string, string][] = [
    // **النمرةُ صارت `settings` لا `profile`** — وليس تليينًا للحارس:
    //   قِيس فوُجد أنّ `users` بلا عمود هاتف، و`ProfilePage` بلا حقل هاتف،
    //   و`/api/auth` يُرسل رمزًا إلى البريد ولا يكتب نمرةً. والنمرةُ
    //   الحقيقيّةُ الوحيدةُ `settings.brand.phone` — تظهر في المتجر وفي
    //   رسالة واتساب. فالبابُ القديمُ كان يفتح على لا شيء.
    ['بغيت نبدل رقم الهاتف ديالي', 'settings'],
...
test('الوجهةُ ترافق كلَّ حكمٍ لا الحكمَ الناجح وحدَه', () => {
  ...
  assert.equal(d.verdict, 'ask');
  assert.equal(d.dest?.page, 'settings', 'ضاعت الوجهةُ مع السؤال');
  // وتحمل معها **الحقلَ** لا الصفحةَ وحدَها: من سمّى النمرةَ يُفتَح له حقلُها.
  assert.equal(d.dest?.focus, 'phone', 'الوجهةُ بلا حقلٍ — تُفتَح صفحةٌ كاملةٌ لمن طلب خانة');
});
