...
//   `normArabic` تُفرّغ ما هو **خارج** نطاق العربيّة، و«،» و«؟» و«؛» **داخله**
//   في يونيكود — فتنجو وتلتصق كأنّها حروف. والنقطةُ لاتينيّةٌ فتُفرَّغ.
//   رمزان يفعلان الشيءَ نفسَه وسلوكان مختلفان.
//
//   والمغاربةُ يكتبون «بغيت سباك،كهربائي» بلا مسافة.
// ============================================================
test('الفاصلةُ العربيّةُ لا تلتصق بالكلمة', () => {
  assert.equal(normArabic('عندي مشكل،سباك'), 'عندي مشكل سباك');
  assert.equal(normArabic('سباك؟'), 'سباك');
  assert.equal(normArabic('بغيت سباك؛كهربائي'), 'بغيت سباك كهربايي');
  // و`normLoose` كذلك — يقرؤها الاتّجاهُ والحقائقُ و`needEngine`.
  assert.equal(normLoose('عندي مشكل،سباك'), 'عندي مشكل سباك');
});

test('والرقمُ العربيُّ يبقى رقمًا — لا يُفرَّغ مع الترقيم', () => {
  // ٠-٩ داخلَ نطاق العربيّة أيضًا. توسيعُ التفريغ إليها يمحو الأثمانَ
  // والكمّيّاتِ من كلّ جملة.
  assert.match(normArabic('بشحال ١٢٠ درهم'), /١٢٠|120/);
});
