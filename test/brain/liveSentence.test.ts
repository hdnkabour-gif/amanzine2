...
// ============================================================
// **ثلاثةُ أعطابٍ رآها صاحبُ المشروع على شاشته** — وكلُّها من هذا البناء.
//
//     «نبدل رقم الهاتف»  ⇒  ومعها **📦 حلاق · الثمن دابا ٠ درهم**
//
//   تبديلُ نمرةِ الإنسان عُرض ومعه منتوجٌ لا علاقةَ له به. وأخطرُ منه ما لم
//   يظهر في اللقطة: `fillSlot` تكتب `need === 'price' ? {price} : {stock}`
//   — فنمرةُ الهاتف كانت **ستُحفَظ ستوكًا** لو مرّت.
// ============================================================

test('**الموضوعُ يُرفَق بحقلٍ يخصّه** — والنمرةُ ليست منتوجًا', () => {
  const phone = build('نبدل رقم الهاتف', P);
  assert.equal(phone.u.action?.object, 'phone', 'تغيّرت القراءةُ — لم تعد هذه الحالةَ المقيسة');
  assert.equal(phone.ls, null, 'عُرض منتوجٌ مع تبديل نمرة — والحفظُ كان سيكتبها ستوكًا');
  assert.equal(build('بغيت نبدل العنوان', P).ls, null, 'العنوانُ فُتح له بابٌ ثانٍ بجانب `FocusedEdit`');
});

test('ولا جملةَ حيّةً لحقلِ منتوجٍ بلا منتوجٍ معلوم', () => {
  // الجملةُ تنفع لأنّها **تُغني عن السؤال**. فبلا منتوجٍ يعود السؤالُ ولا
  // نفعَ فيها — وعرضُ فراغٍ فوق سؤالٍ يُربك ولا يختصر.
  assert.equal(build('بغيت نبدل ثمن المنتوج').ls, null, 'بُنيت جملةٌ بلا منتوجٍ تُغني عن السؤال');
  assert.ok(build('بغيت نبدل ثمن المنتوج', P).ls, 'ما بقاتش تُبنى ومعها منتوج');
});
