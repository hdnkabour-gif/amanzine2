...
// ============================================================
// المصرفُ في **فهرس المفاهيم** — لا في جدول البضائع.
//
//   الحارسُ أعلاه يمنع المطابقةَ داخلَ كلمةٍ في جدول البضائع. وقياسٌ على
//   جملِ إدارة الحساب كشف أنّ **الفهرسَ نفسَه** كان يفعلها، وهو أخطر لأنّه
//   يحكم كلَّ جملة:
//
//       «وريني الز**بناء** ديالي» ⇒ **بنّاء**  — يطلب زبناءه فيُعرَض عليه معلّمُ بناء
//
//   والعربيّةُ تلصق سوابقَها («الحلاق» · «للحلاق») فلا تصلح الكلمةُ الكاملة.
//   فالشرطُ على **البداية وحدَها**: حدُّ كلمةٍ بعد سوابقَ معروفة.
// ============================================================

test('المفهومُ لا يُطابَق في وسط كلمة', () => {
  assert.notEqual(resolveConcept('بغيت نشوف الزبناء ديالي')?.id, 'mason', '«الزبناء» ليست «بنّاء»');
  assert.equal(resolveConcept('الزبناء'), null);
});

test('والسوابقُ الملتصقةُ تبقى مقبولة — العربيّةُ تلصق', () => {
  // لو شُدِّد الشرطُ إلى كلمةٍ كاملةٍ لسقط نصفُ الفهم: الناسُ يكتبون
  // «الحلاق» و«للحلاق» و«وبالحلاق»، ويكتبون «حلاقين» بلاحقةٍ أيضًا.
  for (const q of ['بغيت حلاق', 'بغيت الحلاق', 'مشيت للحلاق', 'فين كاين شي حلاق']) {
    assert.equal(resolveConcept(q)?.id, 'barber', `«${q}» سقطت بالتشديد`);
  }
  assert.equal(resolveConcept('بغيت سباك')?.id, 'plumber');
  assert.equal(resolveConcept('المكتبة')?.id, 'bookstore');
});

test('«رقم الهاتف» حقلٌ في الحساب لا حرفةٌ في السوق', () => {
  // «الهاتف» متغيّرٌ **صحيح** لتصليح الهواتف، لكنّ ما قبله يقلب معناه —
  // كما «ديال» تجعل ما بعدها بضاعةً لا وعاء. وبلا هذا كان من يريد تبديلَ
  // نمرته يُفهَم أنّه يطلب مُصلِّحَ هواتف.
  assert.equal(understand('بغيت نبدل رقم الهاتف ديالي').service, undefined);
  // والحرفةُ نفسُها تبقى مفهومةً حين تُطلَب حقًّا.
  assert.equal(understand('بغيت نصلح الهاتف').service, 'mobile_phone_repair');
  assert.equal(resolveConcept('الهاتف')?.id, 'mobile_phone_repair');
});
