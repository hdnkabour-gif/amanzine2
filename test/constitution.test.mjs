...
test('④ حرفةٌ بلا اتّجاه ⇒ سؤالٌ بخيارَين لا صفحةُ نشر', () => {
  ...
  //
  //   كان الشرطُ مكتوبًا هنا **حرفًا بحرف**: `stance === 'offer' && has(t, p.kw)`.
  //   فلمّا شُدّد الفرعُ بشرطٍ ثالثٍ (ألّا تكون الجملةُ إعلانًا: «عندي محل
  //   للكراء») سقط الاختبارُ — والقيدُ الذي يحرسه لم يُمَسّ بل ازداد.
  //   «اختبارٌ يفشل حين تتحسّن البيئةُ اختبارٌ خاطئ.» يُقاس القيدُ لا نصُّه.
  const iam = need.slice(need.indexOf('for (const p of IAM_PRO)'),
                         need.indexOf('for (const p of IAM_PRO)') + 220);
  assert.ok(/stance === 'offer'/.test(iam) && /has\(t, p\.kw\)/.test(iam),
