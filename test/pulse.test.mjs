...
test('**والمعفاةُ تُعفى من المقام لا من الحراسة**', () => {
  // كان الإعفاءُ `else if`: معفاةٌ لا تُعدّ في البسط إطلاقًا. وسبرٌ أسقط
  // ذلك — كُسرت قراءةُ الاتّجاه في **كلّ** جملةٍ معفاةٍ فلم يتحرّك النبضُ
  // ولا سقّاطتُه ولا العقلُ كلُّه. وخمسٌ من السبع اتّجاهُها مقروءٌ صحيحًا.
  //   فالاستثناءُ لم يكن يُضخّم النسبةَ — لكنّه كان **يُعمي** عنها، وهو
  //   عطبٌ من صنفٍ آخرَ لا يقلّ: ما لا يُقاس لا يُحرَس.
  //
  // والعقدُ: يُرفَع من المقام مَن كان معفًى **ومجهولًا** معًا. فمعفاةٌ
  // اتّجاهُها مقروءٌ نجاحٌ كامل، وطرحُها من المقام وحدَه أعطى ١٠٤٪.
  assert.ok(p.stanceKnown <= p.sentences - p.stanceMoot,
    `نسبةٌ فوق المئة: ${p.stanceKnown}/${p.sentences - p.stanceMoot} — مقياسٌ يكذب لصالحنا`);
  assert.ok(p.stanceKnown + p.stanceMoot <= p.sentences,
    `البسطُ والمعفاةُ يتجاوزان الجملَ: ${p.stanceKnown}+${p.stanceMoot} > ${p.sentences} — عُدّت جملةٌ مرّتين`);
});

test('ولا حلقةَ تتجاوز مقامَها — مقياسٌ فوق المئة يكذب', () => {
  // حارسٌ عامٌّ لكلّ الحلقات لا للاتّجاه وحدَه: أوّلُ عَرَضٍ لخللٍ في المقام
  // أن تتجاوز النسبةُ ١٠٠٪. ولا حلقةَ اليومَ كذلك، والحارسُ يُبقيها كذلك.
  for (const s of p.stages) {
    assert.ok(s.ok <= s.of, `«${s.name}»: ${s.ok}/${s.of} — البسطُ أكبرُ من المقام`);
    assert.ok(s.of > 0, `«${s.name}»: مقامٌ صفرٌ — نسبةٌ بلا معنى`);
  }
});
