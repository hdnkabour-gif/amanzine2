...
// ============================================================
// **مسارٌ مُعلَنٌ لا يناديه أحد.**
//
//   الحارسان أعلاه يسألان: أيوجد مسارٌ بهذا الاسم؟ أيفعل الفعلَ الموعود؟
//   وكلاهما ينظر إلى **الخادم**. وبقي السؤالُ الثالثُ بلا حارس: أيصل إليه
//   أحدٌ من التطبيق أصلًا؟
//
//   ومسحٌ واحدٌ أجاب: مساران يعملان على الخادم بلا مُنادٍ واحدٍ في `src/`.
//   · `/api/payment` — تُنشَأ الدفعاتُ ولا تُقرَأ، والفهرسُ `idx_payments_user`
//     مبنيٌّ منذ الهجرة الأولى لاستعلامٍ لم يُكتب.
//   · `/api/delivery-auto` — أتمتةُ لوحة الشركة، ومحوّلُ السجلّ يقول صراحةً
//     إنّ التنفيذَ الفعليَّ هناك، ولا زرَّ يبلغه.
//
//   والفرقُ بين هذا الحارس و`disconnected.mjs` أنّ ذاك يقيس **الملفّات**
//   التي لا يصلها استيراد، وهذا يقيس **الوعدَ**: قدرةٌ مُعلَنةٌ للإنسان
//   تشير إلى مسارٍ لا يبلغه شيء. الملفّ موصولٌ والوعدُ مقطوع.
// ============================================================

/**
 * كلُّ ملفّات `src` كنصٍّ واحد — **بلا تعليقات**، وعدا الكتالوج نفسِه.
 *
 *   والتعليقاتُ تُحذف لأنّ سبرًا أسقط الحارسَ قبل أن يُسلَّم: حُذفت نداءاتُ
 *   `/api/payment` كلُّها من `api.ts` ومرّ الحارسُ سالمًا — لأنّ **تعليقًا
 *   كتبتُه بيدي في `WalletPage.tsx`** يذكر «`/api/payment`» شرحًا للعطب.
 *   فكان الحارسُ يقرأ كلامًا عن الباب ويحسبه بابًا.
 *
 *   وهذا صنفُ خطأٍ متكرّرٌ في هذا المستودع: حارسٌ يقرأ الملفَّ كلَّه يُصدّق
 *   شرحَ العطب. والذِّكرُ في نثرٍ ليس نداءً أبدًا.
 */
function srcCorpus() {
  ...
  (function walk(dir) {
    for (const e of readdirSync(dir, { withFileTypes: true })) {
      ...
      else if (/\.tsx?$/.test(e.name) && !p.endsWith('abilities.ts')) out.push(strip(readFileSync(p, 'utf8')));
    }
  })(join(ROOT, 'src'));
  return out.join('\n');
}

test('كلُّ مسارٍ مُعلَنٍ يناديه أحدٌ من التطبيق', () => {
  const corpus = srcCorpus();
  //   يُقبَل الشكلان معًا: `request('GET', '/payment')` عبر طبقة الـAPI،
  //   و`fetch('/api/listings/...')` مباشرةً. ومطابقةُ أحدهما وحدَه كانت
  //   تُسقط خمسةَ مساراتٍ سليمة — سبرٌ مرّ لهذا السبب قبل أن يُصحَّح.
  const called = (seg) => new RegExp(
    `['"\`]/(api/)?${seg.replace(/[-/\\^$*+?.()|[\]{}]/g, '\\$&')}['"\`/?$]`
  ).test(corpus);

  //   استثناءاتٌ **فارغةٌ عمدًا**. كلُّ سطرٍ يُضاف هنا دَينٌ مُعلَن، وأيُّ
  //   سطرٍ بلا سببٍ مكتوبٍ يجعل الحارسَ زينة.
  const EXEMPT = new Map([]);

  const orphans = [...new Set(ABILITIES.filter(a => a.api).map(a => a.api))]
    .map(api => api.replace('/api/', ''))
    .filter(seg => !called(seg) && !EXEMPT.has(seg));

  assert.deepEqual(orphans, [],
    `مساراتٌ تُعلنها قدراتٌ ولا يبلغها شيءٌ من الواجهة — الوعدُ مُعلَنٌ والبابُ مقطوع:\n  ${
      orphans.map(s => `/api/${s} ⇐ ${ABILITIES.filter(a => a.api === `/api/${s}`).map(a => a.id).join(', ')}`).join('\n  ')}`);
});

test('والحارسُ يرى النداءَ فعلًا — لا يمرّ لأنّه أعمى', () => {
  // حارسُ الحارس: لو انكسرت المطابقةُ يومًا لصارت النتيجةُ «لا يتيمَ» دائمًا
  // وهي أخطرُ من يتيمٍ مكشوف. نتحقّق أنّ مسارًا لا وجودَ له **يُكتشَف**.
  const corpus = srcCorpus();
  const called = (seg) => new RegExp(
    `['"\`]/(api/)?${seg.replace(/[-/\\^$*+?.()|[\]{}]/g, '\\$&')}['"\`/?$]`
  ).test(corpus);
  assert.equal(called('orders'), true, 'المطابقةُ لا ترى نداءً قائمًا — الحارسُ أعمى');
  assert.equal(called('zzz-no-such-route'), false, 'المطابقةُ ترى نداءً غيرَ موجود — الحارسُ يقول «تمام» دائمًا');
  // وأنّ التعليقاتِ محذوفةٌ فعلًا: `WalletPage.tsx` يذكر «/api/payment» في
  // شرحِ العطب، وكان ذلك وحدَه يُمرّر الحارسَ بعد حذف كلّ النداءات.
  assert.ok(!/كتابةٌ لا تُقرأ|تُعلن `\/api\/wallet`/.test(corpus),
    'التعليقاتُ لم تُحذَف من النصّ المفحوص — شرحُ العطب يُقرأ نداءً');
});
